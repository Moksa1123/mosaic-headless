...
BROWSER      2,473 computed-style readings on the delivered page in Chromium, at
             three viewports: every declared property vs `getComputedStyle` on the
             node it targets. 1,837 compared and agreed, 636 not-comparable and
...
RWD          608 responsive declarations across two sites asserted against the
...
| `data/rwd-verification.csv` | 608 | **checked live** - every `_t`/`_m` declaration vs the served stylesheet, with status per row |
| `data/browser-verification.csv` | 2473 | **computed in Chromium** - declared vs `getComputedStyle` at three viewports, `not-comparable` labelled per row |

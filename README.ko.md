...
| **반응형** | 두 사이트 합쳐 664개의 `_t`/`_m` 선언을, 사이트가 실제로 내보낸 스타일시트에 대해 하나씩 단언 — 전부 검증됨 |
...
| **브라우저** | 전달된 페이지를 Chromium의 세 가지 뷰포트에서 계산 스타일 3,247건 판독: 2,533건 일치, 714건은 비교 불가로 명시, **덮어쓰기 0건** |

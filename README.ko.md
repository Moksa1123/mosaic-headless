...
| **반응형** | 두 사이트 합쳐 608개의 `_t`/`_m` 선언을, 사이트가 실제로 내보낸 스타일시트에 대해 하나씩 단언 — 전부 검증됨 |
...
| **브라우저** | 전달된 페이지를 Chromium의 세 가지 뷰포트에서 계산 스타일 2,473건 판독: 1,837건 일치, 636건은 비교 불가로 명시, **덮어쓰기 0건** |

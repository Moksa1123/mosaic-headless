...
| **レスポンシブ** | 2 サイト計 608 件の `_t`/`_m` 宣言を、サイトが実際に配信したスタイルシートに対して 1 件ずつ断言 — 全件検証済み |
...
| **ブラウザ** | 配信されたページを Chromium の 3 つのビューポートで計算スタイル 2,473 件読み取り：1,837 件が一致、636 件は比較不能として明示、**上書き 0 件** |
